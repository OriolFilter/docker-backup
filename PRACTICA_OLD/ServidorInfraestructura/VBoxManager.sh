...
if VBoxManage list vms | grep -q "$NAMEVM" ; then # Si existeix una màquina amb el mateix nom preguntem si volem borrar
	printf "Ja existeix una màquina amb el nom seleccionat, vols substituïr-la? (S/N)";read -e RESP
...
vbNICS="--nic1 nat --nictype1 virtio --nic2 intnet --nictype2 virtio --intnet2 CONFINAMENT"  # Variable Holder 2
